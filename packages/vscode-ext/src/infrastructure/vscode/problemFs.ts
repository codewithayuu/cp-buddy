...
export class ProblemFs implements IProblemFs {
  public readonly scheme = 'cpbuddy';
  ...
  public constructor(
    ...
    this.signals.on('patchProblem', (srcPath: string) => {
      ...
      this.changeEmitter.fire([
        { uri: Uri.joinPath(baseUri, 'problem.cpbuddy.json'), type: FileChangeType.Changed },
      ]);
    });
    ...
    this.signals.on(
      ...
      (srcPath: string, testcaseId: TestcaseId, payload: Partial<Testcase | TestcaseResult>) => {
        ...
        const problemUri = Uri.joinPath(baseUri, 'problem.cpbuddy.json');
        this.changeEmitter.fire([
