...
>();
const pollingClients = new Set<(msg: any) => void>();

export const emitToEditors = (event: string, payload: any) => {
  io.to('editor-clients').emit(event, payload);
  if (event === 'batchAvailable' || event === 'readingBatch') {
    for (const resolve of pollingClients) resolve({ event, payload });
...
export const broadcastLog = (level: LogLevel, message: string, details: unknown) => {
  emitToEditors('log', { message, level, details });
};
...
app.get('/', (ctx) => {
  ...
  const editorRoom = io.sockets.adapter.rooms.get('editor-clients') || new Set();
  const html = `
    <html>
      <head>
        <title>CPBuddy Router Status</title>
        <style>
          pre {
      ...
      <body>
        <h1>CPBuddy Router Status</h1>
        <p>If you are seeing this, it means that the CPBuddy router has successfully started.</p>
        ...
        <h3>Connected Editor Clients</h3>
        <p>Count: ${editorRoom.size} client(s)</p>
        <ul>
          ${[...editorRoom.values()]
            .map(
              (clientId) => `
...
app.get('/api/poll', async (ctx) => {
  return new Promise((resolve) => {
    let isResolved = false;
    const timeout = setTimeout(() => {
      if (!isResolved) {
        isResolved = true;
        pollingClients.delete(callback);
        resolve(ctx.json({ status: 'timeout' }));
      }
    }, 30000);
    ...
    const callback = (msg: any) => {
      if (!isResolved) {
        ...
        resolve(ctx.json({ status: 'event', ...msg }));
      }
    };
    pollingClients.add(callback);
  });
});
...
app.post('/api/submit', async (ctx) => {
  ...
  if (activeBrowserId) {
    io.to(activeBrowserId).emit('submitRequest', body);
    info('Submission forwarded to browser extension via REST', { body });
    return ctx.json({ status: 'ok' });
  } else {
    error('No browser connected');
    return ctx.json({ status: 'error', message: 'No browser connected' }, 400);
  }
});
app.post('/', async (ctx) => {
  ...
  if (batch.problems.length >= size) {
    if (!batch.ignored) {
      emitToEditors('batchAvailable', {
        batchId: id,
        problems: [...batch.problems],
        autoImport: io.sockets.adapter.rooms.get('editor-clients')?.size === 1 || pollingClients.size > 0,
      });
...
const startServer = () => {
  ...
  io.on('connection', (socket) => {
    ...
    if (type === 'editor') {
      socket.join('editor-clients');
      info('Editor connected', { id: socket.id });
      ...
      socket.on('claimBatch', ({ batchId }) => {
        emitToEditors('batchClaimed', { batchId });
      });
